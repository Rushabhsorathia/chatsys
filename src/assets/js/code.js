...
function initClientChannel() {
    if (currentChannelId()) {
        ...
        subscribedChannels.push(currentChannelId());
    }
}
...
function listenAllContactChannels() {
    ...
    const channel = pusher.subscribe(`${channelName}.123-321-123-321-123-321`);
...
function _listenChannelEvent(channel) {
    ...
    channel.bind("new-group", function (data) {
        const userArray = data.users_id;
        if (userArray.includes(parseInt(auth_id))) {
            const channel = pusher.subscribe(`${channelName}.${data.group_id}`);
            _listenChannelEvent(channel);
            subscribedChannels.push(data.group_id);
            if (data.to_channel_id == currentChannelId() && data.from_id != auth_id) {
                $(".messages").find(".message-hint").remove();
                messagesContainer.find(".messages").append(data.message);
                scrollToBottom(messagesContainer);
    ...
    channel.bind("new-user", function (data) {
        if (!subscribedChannels.includes(data.to_channel_id) && data.to_id == auth_id) {
            const channel = pusher.subscribe(`${channelName}.${data.to_channel_id}`);
            _listenChannelEvent(channel);
            subscribedChannels.push(data.to_channel_id);
...
function showBrowserNotification(sendername, message) {
    var messageText = $(message).find(".message").text().trim();
